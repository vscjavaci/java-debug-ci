...
export function execStdoutWithoutError(command, timeout) {
    return execShort(command, timeout).then(result => {
        return result.stdout;
    }, error => {
        console.error(`Unexpected error when executing ${command} : ${error.message}. ${error.stack}`);
    });
}
...
export function execStdout(command, timeout) {
    return execShort(command, timeout).then(result => {
        ...
        throw new Error(`Unexpected error when executing ${command} : ${error.message}. ${error.stack}`);
    });
}
